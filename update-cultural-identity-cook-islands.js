// Script to update cultural_identity from "Cook Islands" to "Cook Islands Maori" for all businesses
...
async function updateCulturalIdentity() {
  ...
  try {
    ...
    console.log('Finding businesses with cultural_identity containing "Cook Islands"...');
    
    // First, find all businesses with "Cook Islands" in their cultural_identity
    const findResult = await pool.query(`
      SELECT id, business_name, business_handle, cultural_identity 
      FROM businesses 
      WHERE cultural_identity::text LIKE '%Cook Islands%'
      ORDER BY business_name
    `);
    
    console.log(`\nFound ${findResult.rows.length} businesses with "Cook Islands" in cultural_identity:`);
    
    if (findResult.rows.length === 0) {
      console.log('No businesses found with "Cook Islands" in cultural_identity');
      return;
    }
    ...
    console.log('\nUpdating cultural_identity from "Cook Islands" to "Cook Islands Maori"...');
    ...
    for (const business of findResult.rows) {
      ...
      if (business.cultural_identity) {
        try {
          ...
          // Replace "Cook Islands" with "Cook Islands Maori" in the array
          identities = identities.map(identity => {
            if (identity === "Cook Islands") {
              return "Cook Islands Maori";
            }
            return identity;
          });
