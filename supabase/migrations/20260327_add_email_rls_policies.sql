...
-- Allows admin users to manage email campaigns and subscribers

-- Enable RLS on all email tables (if not already enabled)
...
CREATE POLICY "Admins can insert subscribers" ON email_subscribers
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can update subscribers" ON email_subscribers
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can insert campaign recipients" ON email_campaign_recipients
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can delete campaign recipients" ON email_campaign_recipients
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can view campaign queue" ON email_campaign_queue
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can insert campaign queue" ON email_campaign_queue
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can update campaign queue" ON email_campaign_queue
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can delete campaign queue" ON email_campaign_queue
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
-- Subscriber entities policies
CREATE POLICY "Admins can view subscriber entities" ON email_subscriber_entities
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can insert subscriber entities" ON email_subscriber_entities
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can update subscriber entities" ON email_subscriber_entities
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can delete subscriber entities" ON email_subscriber_entities
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
-- Events policies
CREATE POLICY "Admins can view all events" ON email_events
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Admins can insert events" ON email_events
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
