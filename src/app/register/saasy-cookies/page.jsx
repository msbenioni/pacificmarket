...
export default function SaasyCookiesRegister() {
  ...
  const referralCode = "saasy-cookies";

  useEffect(() => {
    // Load referrer business info for display
    loadReferrerBusiness(referralCode);
  }, []);
  ...
  const loadReferrerBusiness = async (handle) => {
    try {
      ...
      console.log('Referrer business query result:', { data, error });
      ...
      if (data) {
        ...
        console.log('Referrer business loaded:', data);
      } else {
        setError(`Business not found: ${handle}. Please check the referral link.`);
      }
    } catch (error) {
