# Supabase Migration Script (PowerShell)
...
function Test-Connection {
    ...
    try {
        ...
        Write-Host "🔗 Testing connection to $DbHost`:$DbPort/$Database" -ForegroundColor Blue
...
function Invoke-SQLFile {
    ...
    Write-Host " Running migration: $(Split-Path $FilePath -Leaf)" -ForegroundColor Blue
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host " Migration completed: $(Split-Path $FilePath -Leaf)" -ForegroundColor Green
    }
    else {
        Write-Host " Migration failed: $(Split-Path $FilePath -Leaf)" -ForegroundColor Red
...
"@
    ...
    Write-Host "✅ Found $($Tables.Count) expected tables:" -ForegroundColor Green
    ...
    Write-Host "✅ Scheduler config accessible: $ConfigTest records" -ForegroundColor Green
}
...
try {
    ...
    if (-not $ConnectionString) {
        Write-Host "❌ SUPABASE_DB_CONNECTION_STRING not found in environment variables" -ForegroundColor Red
    ...
    Write-Host "2. Visit: http://localhost:3000/admin/client-list-manager" -ForegroundColor Gray
    Write-Host "3. Test the discovery workflow" -ForegroundColor Gray
    Write-Host "" -ForegroundColor White
    Write-Host "🔒 For production security, run:" -ForegroundColor Yellow
