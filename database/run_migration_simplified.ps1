...
# Simplified schema - core functionality only
# ================================================================

param(
    [Parameter(Mandatory=$false)]
    [string]$Action = "migrate",  # Options: backup, migrate, verify, cleanup, rollback
    [Parameter(Mandatory=$false)]
    [string]$SchemaFile = "pacific_market_simplified_schema.sql",
    [Parameter(Mandatory=$false)]
...
Write-Host "Pacific Market Database Migration (Simplified)" -ForegroundColor Green
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'ℹ️ Business Insights table not found, skipping backup';
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'ℹ️ Founder Insights table not found, skipping backup';
    END;
...
"@
    ...
    "cleanup" {
        Write-Host "Cleaning up unused tables and indexes..." -ForegroundColor Cyan
        
        $CleanupPath = Join-Path $ProjectRoot "database\cleanup_unused_tables.sql"
        
        Write-Host "Cleanup file: $CleanupPath" -ForegroundColor White
        
        try {
            $result = & psql $ConnectionString -f $CleanupPath
            if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "❌ Exception: Cleanup failed" -ForegroundColor Red
            Write-Host "Error: $($_.Exception.Message)" -ForegroundColor Red
            throw
        }
    }
...
"@
    ...
    default {
        ...
        Write-Host "  cleanup  - Remove unused tables and indexes" -ForegroundColor White
