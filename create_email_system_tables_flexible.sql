-- Enhanced Email System with Flexible Subscriber-Business Relationships
-- This supports multiple businesses per subscriber and other entity types
...
DROP TABLE IF EXISTS email_subscriber_entities CASCADE; -- New relationship table
DROP TABLE IF EXISTS email_templates CASCADE;

-- Core email subscribers table (email-centric, not business-centric)
...
-- New table: Link subscribers to multiple entities (businesses, creator listings, etc.)
CREATE TABLE IF NOT EXISTS email_subscriber_entities (
    ...
    entity_type VARCHAR(50) NOT NULL, -- 'business', 'creator_listing', 'charity', 'community'
    entity_id UUID NOT NULL, -- References businesses.id, creator_listings.id, etc.
    entity_name VARCHAR(255), -- Cached name for display
    relationship_type VARCHAR(50) DEFAULT 'owner', -- owner, manager, member, contact
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TABLE IF NOT EXISTS email_campaigns (
    ...
    audience VARCHAR(50) NOT NULL, -- all, business_owners, mana_plan, moana_plan, referral_participants, creator_listings
...
-- Clean up existing policies (after tables exist)
DROP POLICY IF EXISTS "Anyone can view email_subscribers" ON email_subscribers;
DROP POLICY IF EXISTS "Anyone can view email_subscriber_entities" ON email_subscriber_entities;
...
CREATE POLICY "Anyone can view email_subscriber_entities" ON email_subscriber_entities
    FOR SELECT USING (true);

CREATE POLICY "Admins full access to email_subscriber_entities" ON email_subscriber_entities
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE profiles.id = auth.uid() 
...
CREATE INDEX IF NOT EXISTS idx_email_subscriber_entities_entity ON email_subscriber_entities(entity_type, entity_id);
...
CREATE INDEX IF NOT EXISTS idx_email_campaign_recipients_subscriber_id ON email_campaign_recipients(subscriber_id);
...
-- Add comments
COMMENT ON TABLE email_subscribers IS 'Master email list - email-centric design';
COMMENT ON TABLE email_subscriber_entities IS 'Links subscribers to multiple entities (businesses, creator listings, charities, etc.)';
