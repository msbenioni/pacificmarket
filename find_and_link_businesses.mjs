...
async function findUserAndLinkBusinesses() {
  try {
    console.log('=== Finding user account ===');
    
    // Check the claim requests to see what user_id they use
    ...
    claims?.forEach(c => {
      console.log(`- User ID: ${c.user_id}, Email: ${c.contact_email}`);
    });
    
    if (!claims || claims.length === 0) {
      console.log('No claims found for these businesses');
      return;
    }
    
    const userId = claims[0].user_id;
    ...
    console.log(`\nFound profile: ${profile.display_name} (${profile.email})`);
    ...
    console.log('The claim requests are no longer needed since the businesses are now directly linked to you.');
