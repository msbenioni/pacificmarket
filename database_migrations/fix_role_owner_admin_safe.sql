-- Fix Role System - Owner/Admin (Handles Dependencies Safely)
-- This approach preserves all dependencies while converting the enum
...
-- Step 4: Update the views to use the new column temporarily
-- Drop and recreate views that depend on the role column

-- Drop the duplicate public_profiles view
DROP VIEW IF EXISTS public_profiles CASCADE;

-- Recreate analytics_profiles view to use new column
CREATE OR REPLACE VIEW analytics_profiles AS
SELECT 
    -- Public fields from main profiles table
    id,
    role_new as role,  -- Use new column
    display_name,
    email,
    country,
    primary_cultural,
...
-- Step 5: Handle RLS policies that depend on the role column
-- We need to temporarily disable policies that reference the old role column

-- Get list of policies that depend on profiles.role
-- These will need to be updated after the column swap

-- Step 6: Drop the old role column (with CASCADE to handle dependencies)
ALTER TABLE profiles DROP COLUMN role CASCADE;

-- Step 7: Rename the new column to replace the old one
...
-- Step 10: Recreate the analytics_profiles view with the correct column name
CREATE OR REPLACE VIEW analytics_profiles AS
SELECT 
    -- Public fields from main profiles table
    id,
    role,  -- Now using the correct column name
    display_name,
    email,
    country,
    primary_cultural,
...
-- Step 11: Grant permissions on the updated analytics view
GRANT SELECT ON analytics_profiles TO authenticated;

-- Step 12: Add comment explaining the new role system
...
-- Step 13: Ensure admin users have correct roles
UPDATE profiles SET role = 'admin' WHERE id IN (SELECT owner_user_id FROM admin_users WHERE role = 'admin');

-- Step 14: Recreate RLS policies that were dropped
-- Note: You may need to manually recreate some policies after this migration
...
-- Check which policies might need recreation
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual 
FROM pg_policies 
WHERE tablename IN ('feature_templates', 'platform_settings', 'storage.objects');
